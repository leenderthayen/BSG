...
namespace BSG {
  ...
  bool Generator::InitializeOptionsFromConfigFile(std::string filename) {
    ...
    if (configOptions.robtdMethod == ROBTDMethod::ESP) {
      InitializeESPROBTD();
    } else if (configOptions.robtdMethod == ROBTDMethod::EXTERNAL) {
      //TODO
      InitializeExternalROBTD("");
    }
    return true;
  }
  ...
  std::vector<std::vector<double> >* Generator::CalculateSpectrum() {
    ...
    double stepW = configOptions.spectrumCalcOptions.stepSize / ELECTRON_MASS_KEV;
